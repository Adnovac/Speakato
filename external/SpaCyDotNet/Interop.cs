...
namespace SpacyDotNet
{
    public class Interop
    {
        ...
        public static List<T> GetListFromList<T>(dynamic pyList, ref List<T> lstMember)
        {
            ...
            using (Py.GIL())
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    {
                        created = float.Parse(element.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture);
                    }
                    else
                    {
                        Debug.Assert(false);
        ...
        public static List<T> GetListFromSet<T>(dynamic pySet, ref List<T> lstMember)
        {
            if (lstMember != null)
                return lstMember;
            ...
            using (Py.GIL())
            {
                ...
                dynamic builtins = Py.Import("builtins");
                var pyCount = new PyInt(builtins.len(pySet));
                var count = pyCount.ToInt32();

                foreach (var element in pySet)
                {
